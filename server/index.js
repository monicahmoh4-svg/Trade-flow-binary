...
const IS_DEMO_MODE = !CONSUMER_KEY || !CONSUMER_SECRET || !PASSKEY ||
  CONSUMER_KEY === "your_consumer_key_from_daraja";

console.log(`[MPESA] env=${MPESA_ENV} demoMode=${IS_DEMO_MODE} shortcode=${SHORTCODE}`);
if(IS_DEMO_MODE) console.log("[MPESA] Running in DEMO mode — set MPESA_* env vars for real payments");
...
// seed admin
DB.users.push({
  id:"admin-001", username: process.env.ADMIN_USERNAME||"admin",
...
function formatPhone(raw){
  ...
  if(/^[71]/.test(p))   p = "254" + p;
  return p;
}

// STK timestamp + password
function stkParams(){
...
// ── Daraja: get OAuth token using Node https (no external libs) ──
function darajaGet(urlPath, authHeader){
  return new Promise((resolve, reject) => {
    const url = new URL(DARAJA_BASE + urlPath);
    const opts = {
...
function darajaPost(urlPath, token, payload){
  return new Promise((resolve, reject) => {
...
async function getToken(){
  ...
  const r = await darajaGet(
    "/oauth/v1/generate?grant_type=client_credentials",
    `Basic ${creds}`
  );
  console.log("[DARAJA] OAuth response:", r.status, JSON.stringify(r.body));
  if(!r.body?.access_token){
    throw new Error(`OAuth failed (${r.status}): ${JSON.stringify(r.body)}`);
  }
...
app.post("/api/payments/deposit", authMiddleware, async(req,res)=>{
  ...
  if(!/^2547\d{8}$|^2541\d{8}$/.test(fmtPhone))
    return res.status(400).json({
      error:`Invalid phone number "${fmtPhone}". Use 07XXXXXXXX or 01XXXXXXXX`,
    });
  ...
  if(IS_DEMO_MODE){
    txn.status = "demo";
    console.log("[DEPOSIT] Demo mode — auto-crediting on poll");
    return res.json({
      transactionId: txn.id,
      mode: "demo",
      message: "Demo mode — no Daraja credentials set. Deposit will be credited automatically.",
    });
  }

  // ── REAL DARAJA ───────────────────────────────────────────
  try{
    console.log("[DARAJA] Getting OAuth token...");
    const token = await getToken();
    console.log("[DARAJA] Token obtained. Sending STK push...");

    const { ts, pw } = stkParams();
    const payload = {
      ...
      TransactionDesc:   `Deposit KES ${Math.ceil(amt)}`,
    };

    console.log("[DARAJA] STK payload:", JSON.stringify({...payload, Password:"[HIDDEN]"}));

    const r = await darajaPost("/mpesa/stkpush/v1/processrequest", token, payload);

    console.log("[DARAJA] STK response:", r.status, JSON.stringify(r.body));

    // Daraja success = ResponseCode "0"
    if(r.body?.ResponseCode==="0"){
      txn.mpesaCheckoutId = r.body.CheckoutRequestID;
      txn.notes = r.body.CustomerMessage||"STK push sent";
      console.log("[DARAJA] STK push success. CheckoutID:", txn.mpesaCheckoutId);
      return res.json({
        transactionId:    txn.id,
        checkoutRequestId: r.body.CheckoutRequestID,
        message:           r.body.CustomerMessage||"Check your phone and enter your M-Pesa PIN",
      });
    }

    // Daraja returned an error in body
    txn.status = "failed";
    const errMsg = r.body?.errorMessage || r.body?.ResponseDescription || r.body?.ResultDesc || JSON.stringify(r.body);
    txn.notes = errMsg;
    console.error("[DARAJA] STK push failed:", r.status, errMsg);
    return res.status(502).json({
      error:  "M-Pesa STK push failed",
      detail: errMsg,
      code:   r.body?.errorCode||r.body?.ResponseCode,
    });

  }catch(e){
    txn.status = "failed";
    txn.notes  = e.message;
    console.error("[DARAJA] Exception:", e.message);
    return res.status(502).json({
      error:  "STK initialization failed",
      detail: e.message,
    });
  }
});

// ── MPESA — Poll deposit status ───────────────────────────────
app.get("/api/payments/deposit/:txnId/status", authMiddleware, async(req,res)=>{
  ...
  try{
    const token = await getToken();
    const {ts,pw} = stkParams();
    const r = await darajaPost("/mpesa/stkpushquery/v1/query", token,{
      BusinessShortCode: SHORTCODE,
      Password:          pw,
      Timestamp:         ts,
    ...
    console.log("[DARAJA] Query response:", r.status, JSON.stringify(r.body));

    const rc = String(r.body?.ResultCode??"");

    if(rc==="0"){
      // success
      const meta  = r.body?.CallbackMetadata?.Item||[];
      ...
      const u = DB.users.find(x=>x.id===req.user.id);
      if(u&&txn.status!=="success"){ u.balance+=txn.amount; u.totalDeposited+=txn.amount; }
      // always credit if not yet credited
      if(!txn._credited){
        txn._credited=true;
        const u2 = DB.users.find(x=>x.id===req.user.id);
        if(u2){ u2.balance+=txn.amount; u2.totalDeposited+=txn.amount; }
      }
    } else if(rc==="1032"){ txn.status="cancelled"; txn.notes="Cancelled by user"; }
      else if(rc==="1037"){ txn.status="failed"; txn.notes="Timed out — no response"; }
      else if(r.body?.errorCode==="500.001.1001"){
        // still processing — keep polling
        return res.json({status:"pending"});
      } else if(rc&&rc!=="0"){
        txn.status="failed";
        txn.notes = r.body?.ResultDesc||r.body?.errorMessage||"Payment failed";
      }
    ...
    console.error("[DARAJA] Query exception:", e.message);
    return res.json({status:txn.status});
  }
});
...
app.post("/api/payments/mpesa/callback",(req,res)=>{
  try{
    ...
    if(!txn){ console.log("[CALLBACK] No txn for", cb.CheckoutRequestID); return res.json({ResultCode:0,ResultDesc:"OK"}); }
    ...
    if(cb.ResultCode===0){
      ...
      console.log("[CALLBACK] Payment failed/cancelled:", cb.ResultCode, cb.ResultDesc);
    }
...
app.get("/api/admin/stats", authMiddleware, adminOnly,(req,res)=>{
  ...
  const pWds=wds.filter(t=>t.status==="pending");
  const tt=DB.trades.length, wt=DB.trades.filter(t=>t.result==="WIN").length;
  const hp=DB.trades.filter(t=>t.result!=="PENDING").reduce((s,t)=>s+(t.result==="LOSS"?t.stake:-t.payout),0);
  res.json({
    ...
    pendingWithdrawalsCount:pWds.length,
    pendingWithdrawalsAmount:pWds.reduce((s,t)=>s+t.amount,0),
...
app.patch("/api/admin/users/:id", authMiddleware, adminOnly,(req,res)=>{
  const user=DB.users.find(u=>u.id===req.params.id);
  if(!user) return res.status(404).json({error:"Not found"});
  ["balance","status","kycVerified","phone"].forEach(k=>{ if(req.body[k]!==undefined) user[k]=k==="balance"?parseFloat(req.body[k]):req.body[k]; });
  res.json(safeUser(user));
});
app.post("/api/admin/users/:id/adjust-balance", authMiddleware, adminOnly,(req,res)=>{
  ...
  DB.transactions.push({id:uuidv4(),userId:user.id,type:`admin_${type}`,amount:adj,status:"success",notes:reason||"Admin adjustment",createdAt:new Date().toISOString(),completedAt:new Date().toISOString()});
...
app.listen(PORT,()=>{
  console.log(`TradeFlow Pro running on port ${PORT} [${MPESA_ENV.toUpperCase()}]`);
  console.log(`Daraja base: ${DARAJA_BASE}`);
