...
const IS_DEMO = !CONSUMER_KEY || !CONSUMER_SECRET || !PASSKEY ||
  CONSUMER_KEY.includes("your_consumer");

console.log("==============================================");
console.log(`TradeFlow Pro starting — MPESA_ENV: ${MPESA_ENV}`);
console.log(`Demo mode: ${IS_DEMO}`);
console.log(`Daraja base: ${DARAJA_BASE}`);
console.log(`Shortcode: ${SHORTCODE}`);
console.log(`Consumer key set: ${!!CONSUMER_KEY} (${CONSUMER_KEY.slice(0,6)}...)`);
console.log(`Consumer secret set: ${!!CONSUMER_SECRET}`);
...
// Get OAuth token — uses Node 18 global fetch
async function getAccessToken(){
  ...
  console.log(`[OAUTH] status=${res.status} body=${text}`);
  ...
  catch { throw new Error(`OAuth bad JSON: ${text}`); }

  if(!data.access_token){
    throw new Error(`OAuth missing token: ${text}`);
  }

  console.log(`[OAUTH] Token obtained OK (expires_in=${data.expires_in})`);
...
async function stkPush(token, phone, amount){
  ...
  const text = await res.text();
  console.log(`[STK] status=${res.status} body=${text}`);

  let data;
  try { data = JSON.parse(text); }
  catch { throw new Error(`STK bad JSON (${res.status}): ${text}`); }
...
async function stkQuery(token, checkoutRequestId){
  ...
  const text = await res.text();
  console.log(`[QUERY] checkoutId=${checkoutRequestId} status=${res.status} body=${text}`);
...
app.post("/api/payments/deposit", authMiddleware, async(req,res)=>{
  ...
  if(!/^2547\d{8}$|^2541\d{8}$/.test(fmtPhone))
    return res.status(400).json({
      error:`Invalid phone number. Got "${fmtPhone}". Use format 07XXXXXXXX`,
    });
  ...
  if(IS_DEMO){
    txn.status="demo";
    console.log("[DEPOSIT] Demo mode — will auto-credit on poll");
    return res.json({
  ...
  try{
    ...
    const {status, data} = await stkPush(token, fmtPhone, amt);

    if(data.ResponseCode === "0"){
      txn.mpesaCheckoutId = data.CheckoutRequestID;
      txn.notes = data.CustomerMessage || "STK push sent";
      console.log(`[DEPOSIT] STK OK checkoutId=${data.CheckoutRequestID}`);
      return res.json({
        transactionId:     txn.id,
    ...
    // Daraja returned an error body
    txn.status="failed";
    const errMsg = data.errorMessage || data.ResponseDescription || data.ResultDesc || JSON.stringify(data);
    txn.notes = errMsg;
    console.error(`[DEPOSIT] STK failed: ${errMsg}`);
...
app.get("/api/payments/deposit/:txnId/status", authMiddleware, async(req,res)=>{
  ...
  try{
    const token=await getAccessToken();
    const {status,data}=await stkQuery(token, txn.mpesaCheckoutId);
    ...
    }else if(data?.errorCode==="500.001.1001"||data?.errorMessage?.includes("in process")){
      // still processing
...
app.listen(PORT,()=>{
  console.log(`TradeFlow Pro listening on port ${PORT}`);
});
