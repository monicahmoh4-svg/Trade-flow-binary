...
function drawChart(canvas, data, rate, entryRate, direction, tradeOn) {
  ...
  const MAX=0.18, MIN=-0.14, rng=MAX-MIN;
  ...
  gG.addColorStop(0,'rgba(0,210,100,0.5)'); gG.addColorStop(1,'rgba(0,210,100,0.05)');
  ctx.beginPath();
  ...
  rG.addColorStop(0,'rgba(220,40,60,0.05)'); rG.addColorStop(1,'rgba(220,40,60,0.5)');
  ctx.beginPath();
...
export default function TradePage() {
  ...
  // ── KEY FIX: separate ref for the chat scroll CONTAINER (not the end sentinel) ──
  ...
  useEffect(()=>{
    ...
    chartData.current=s; setRate(s[s.length-1]); setTick(t=>t+1);
  },[sel]);
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{
      ...
      chartData.current=nd; setRate(nx); setTick(t=>t+1);
    },270);
  ...
  useEffect(()=>{
    ...
    if(demo){ setDemoBal(b=>won?b+trade.stake+trade.potW:b); setDemoHist(h=>[rec,...h.slice(0,49)]); }
    else { setBal(b=>won?b+trade.stake+trade.potW:b); setHistory(h=>[rec,...h.slice(0,49)]); tradeAPI.place({asset:sel2?.name,direction:trade.dir,stake:trade.stake,expirySec:trade.expSec,expiryLabel:trade.expLabel,entryPrice:sel2?.price}).catch(()=>{}); }
  ...
  // ── CHAT: scroll only the chat box, never the page ──────────
  useEffect(()=>{
  ...
  const ChatContent=()=>(
    <>
      ...
      {/* ── scrollable chat box — only THIS div scrolls ── */}
      <div
        ref={chatBoxRef}
        style={{flex:1,overflowY:'auto',overflowX:'hidden',background:T.s2}}
      >
        {chat.map(m=>{
  ...
  const AssetListContent=({onSelect})=>(
    <>
      ...
      <div style={{display:'flex',gap:4,padding:'6px 8px',borderBottom:`1px solid ${T.border}`,flexWrap:'wrap',flexShrink:0}}>
        {['all','forex','crypto','nse','comm'].map(c=>(
          <button key={c} onClick={()=>setCat(c)} style={pill(cat===c,T.blue,T.bdim)}>
      ...
      <div style={{flex:1,overflowY:'auto',overflowX:'hidden'}}>
        {filtered.map(a=>{
          const s=sel?.id===a.id;
          return(
            <div key={a.id} onClick={()=>{setSel(a);if(onSelect)onSelect();}}
  ...
  return (
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:isMobile?5:10}}>
          <div style={{background:T.s2,border:`1px solid ${T.border2}`,borderRadius:8,padding:isMobile?'3px 8px':'5px 14px',flexShrink:0}}>
            ...
            <div style={{fontFamily:'monospace',fontSize:isMobile?12:15,fontWeight:700,color:demo?T.green:T.gold}}>KES {AB.toLocaleString()}</div>
          </div>
          {!demo?(
            <>
            ...
            <>
              <button onClick={()=>{setDemoBal(10000);setDemoHist([]);setTrade(null);toast('Demo reset','info');}}
      ...
      {/* DEMO BANNER */}
      {demo&&(
        <div style={{background:'#1a1400',borderBottom:`1px solid ${T.gold}44`,padding:'5px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexShrink:0}}>
          ...
          <div style={{display:'flex',gap:6,flexShrink:0}}>
            ...
            <button onClick={()=>{setDemo(false);setSel(assets[0]||null);setTrade(null);}}
              style={{height:26,padding:'0 10px',borderRadius:5,border:`1px solid ${T.gold}55`,background:T.ydim,color:T.gold,fontWeight:700,fontSize:10,cursor:'pointer'}}>→ Live</button>
          </div>
        </div>
      )}
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,display:'flex',padding:`0 ${isMobile?4:16}px`,overflowX:'auto',flexShrink:0}}>
        ...
        {isMobile&&!demo&&(
          <button onClick={()=>setShowWd(true)} style={{marginLeft:'auto',padding:'10px 10px',background:'transparent',border:'none',borderBottom:'3px solid transparent',color:T.gold,fontWeight:700,fontSize:10,cursor:'pointer'}}>💸</button>
        )}
      </div>
      ...
      {/* ── CONTENT AREA — fills remaining height, never overflows ── */}
      <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column',minHeight:0}}>
            ...
            <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
              ...
              {isMobile&&mobTab==='assets'&&(
                <div style={{flex:1,overflow:'hidden',background:T.surface,display:'flex',flexDirection:'column'}}>
                  <AssetListContent onSelect={()=>setMobTab('chart')}/>
                </div>
              )}
                ...
                <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minHeight:0}}>
                  ...
                  {trade&&trade.status==='active'&&(
                    <div className="fade-up" style={{padding:'10px 14px',background:trade.win?T.gdim:T.rdim,borderTop:`2px solid ${trade.win?T.green:T.red}`,borderBottom:`1px solid ${T.border}`,flexShrink:0,display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                      ...
                      <div style={{textAlign:'center'}}>
                        <div style={{fontFamily:'monospace',fontSize:22,fontWeight:900,color:(trade.livePnl||0)>=0?T.green:T.red,lineHeight:1}}>{(trade.livePnl||0)>=0?'+':''}{(trade.livePnl||0).toLocaleString()}</div>
                  ...
                  {trade&&(trade.status==='won'||trade.status==='lost')&&(
                    <div className="fade-up" style={{padding:'14px 18px',background:trade.status==='won'?T.gdim:T.rdim,border:`2px solid ${trade.status==='won'?T.green:T.red}`,margin:'8px 12px',borderRadius:12,display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
                      ...
                      <div>
                        <div style={{fontSize:16,fontWeight:900,color:trade.status==='won'?T.green:T.red}}>{trade.status==='won'?`YOU WIN! +KES ${trade.potW.toLocaleString()}`:`LOSS — KES ${trade.stake.toLocaleString()}`}</div>
                        <div style={{fontSize:11,color:T.muted,marginTop:2}}>{trade.dir} · Entry {trade.entryRate>=0?'+':''}{trade.entryRate.toFixed(4)} → Exit {trade.exitRate>=0?'+':''}{trade.exitRate?.toFixed(4)}</div>
                      </div>
                    </div>
                  )}

                  {/* chart */}
                  <div style={{flex:1,background:'#0e1117',minHeight:isMobile?200:240,overflow:'hidden'}}>
                  ...
                  <div style={{background:T.surface,borderTop:`1px solid ${T.border2}`,padding:isMobile?'10px 12px':'14px 20px',flexShrink:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:10,flexWrap:'wrap'}}>
                      ...
                      <div style={{marginLeft:'auto',fontFamily:'monospace',fontSize:11,background:T.ydim,border:`1px solid ${T.gold}44`,borderRadius:6,padding:'4px 10px',color:T.gold,fontWeight:700,flexShrink:0}}>{pout}% payout</div>
                    </div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                      {[['CALL','▲ CALL (UP)','Price will rise',T.green,`linear-gradient(135deg,#007a30,${T.green})`,'rgba(0,150,80,0.4)'],['PUT','▼ PUT (DOWN)','Price will fall',T.red,`linear-gradient(135deg,#a00020,${T.red})`,'rgba(200,0,40,0.4)']].map(([dir,lbl,sub,col,bg,sh])=>(
                        <button key={dir} onClick={()=>place(dir)} disabled={trade?.status==='active'}
                          style={{padding:isMobile?'13px 8px':'17px 8px',border:'none',borderRadius:10,background:trade?.status==='active'?T.s3:bg,color:trade?.status==='active'?T.muted:'#fff',fontWeight:900,fontSize:isMobile?13:15,cursor:trade?.status==='active'?'not-allowed':'pointer',boxShadow:trade?.status==='active'?'none':`0 4px 20px ${sh}`,lineHeight:1.3,transition:'all .15s'}}>
                          {lbl}<br/><span style={{fontSize:10,fontWeight:400,opacity:.75}}>{sub}</span>
                        </button>
                      ))}
                    </div>
                    {trade?.status==='active'&&<div style={{marginTop:8,textAlign:'center',fontSize:11,color:T.muted,fontStyle:'italic'}}>Trade running — wait for settlement</div>}
                  </div>
                </div>
              )}
            </div>
                  ...
                  <div style={{padding:trade?.status==='active'?0:'16px 12px',textAlign:trade?.status==='active'?'left':'center'}}>
                    {trade?.status==='active'?(
                      <div style={{padding:'10px 12px'}}>
                        ...
                        <div style={{fontFamily:'monospace',fontSize:10,color:T.muted,marginBottom:4}}>KES {trade.stake.toLocaleString()} · +{trade.potW.toLocaleString()} if win</div>
                        ...
                        <div style={{fontFamily:'monospace',fontSize:14,fontWeight:700,color:(trade.livePnl||0)>=0?T.green:T.red,marginBottom:6}}>{(trade.livePnl||0)>=0?'+':''}KES {Math.abs(trade.livePnl||0).toLocaleString()}</div>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:9,color:T.muted2}}>
                          ...
                          <span style={{color:trade.win?T.green:T.red}}>{trade.win?'✅ Win':'❌ Lose'}</span>
                        </div>
                      </div>
                    ):(
                ...
                <div style={{padding:'8px 10px',borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5}}>
                    {(demo?[
                      ...
                      <div key={l} style={{background:T.s2,borderRadius:7,padding:'7px 9px',border:`1px solid ${T.border}`}}>
                        <div style={{fontSize:8,textTransform:'uppercase',letterSpacing:.8,color:T.muted2,marginBottom:2}}>{l}</div>
                        <div style={{fontFamily:'monospace',fontSize:13,fontWeight:700,color:c||T.text}}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
                {/* chat — contained in its own flex column, scroll is internal */}
                <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minHeight:0}}>
                  <ChatContent/>
                </div>
              </div>
        ...
        {/* ══ HISTORY TAB ══ */}
        {tab==='history'&&(
          <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:isMobile?10:24}}>
            ...
            <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:'hidden'}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:isMobile?11:13}}>
                  ...
                  <tbody>
                    {AH.length===0&&<tr><td colSpan={8} style={{textAlign:'center',color:T.muted,padding:40}}>No trades yet</td></tr>}
                    {AH.map(t=>(
                      <tr key={t.id} style={{borderBottom:`1px solid ${T.border}`}}>
                        ...
                        <td style={{padding:'10px 12px',fontFamily:'monospace',fontWeight:800,color:t.payout>=0?T.green:T.red,whiteSpace:'nowrap'}}>{t.payout!=null?`${t.payout>=0?'+':''}KES ${Math.abs(t.payout).toLocaleString()}`:'—'}</td>
        ...
        {tab==='wallet'&&!demo&&(
          <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:isMobile?10:24}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr 1fr':'repeat(3,1fr)',gap:12,marginBottom:20}}>
              {[['Balance',`KES ${bal.toLocaleString()}`,T.gold],['Deposited',`KES ${(stats?.totalDeposited||0).toLocaleString()}`,T.green],['Withdrawn',`KES ${(stats?.totalWithdrawn||0).toLocaleString()}`,T.muted]].map(([l,v,c])=>(
                <div key={l} style={{background:T.surface,border:`1px solid ${T.border2}`,borderRadius:12,padding:16,textAlign:'center'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:24}}>
              <button onClick={()=>setShowDep(true)} style={{padding:16,borderRadius:12,border:'none',background:`linear-gradient(135deg,#009944,${T.green})`,color:'#fff',fontWeight:900,fontSize:isMobile?13:15,cursor:'pointer',boxShadow:'0 4px 20px rgba(0,160,80,0.3)'}}>📱 Deposit via M-Pesa</button>
              <button onClick={()=>setShowWd(true)} style={{padding:16,borderRadius:12,border:`2px solid ${T.gold}55`,background:T.ydim,color:T.gold,fontWeight:900,fontSize:isMobile?13:15,cursor:'pointer'}}>💸 Withdraw to M-Pesa</button>
            </div>
            ...
            <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:'hidden'}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                  <thead>
                    <tr style={{background:T.s2,borderBottom:`2px solid ${T.border2}`}}>
                      {['Type','Amount','Status','Date','Ref'].map(h=>(
                  ...
                  <tbody>
                    ...
                    {txns.map(t=>(
                      <tr key={t.id} style={{borderBottom:`1px solid ${T.border}`}}>
                        ...
                        <td style={{padding:'10px 12px',fontSize:11,color:T.muted2,maxWidth:130,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.mpesaRef||t.notes||'—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab==='stats'&&(
          <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:isMobile?10:24}}>
            <div style={{fontSize:isMobile?14:18,fontWeight:800,marginBottom:16}}>{demo?'Demo Performance':'📊 My Performance'}</div>
            ...
            <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:'hidden'}}>
              ...
              {AH.slice(0,15).map(t=>(
                <div key={t.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 16px',borderBottom:`1px solid ${T.border}`,flexWrap:'wrap',gap:8}}>
                  ...
                  <span style={{fontFamily:'monospace',fontSize:13,fontWeight:700,color:t.payout>=0?T.green:T.red}}>{t.payout!=null?`${t.payout>=0?'+':''}KES ${Math.abs(t.payout).toLocaleString()}`:'—'}</span>
