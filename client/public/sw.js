...
const STATIC_ASSETS = ['/', '/trade', '/login', '/manifest.json'];
...
self.addEventListener('push', e => {
  const data = e.data?.json() || { title: 'TradeFlow Pro', body: 'New update!' };
  e.waitUntil(
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(clients.openWindow(e.notification.data || '/trade'));
});
